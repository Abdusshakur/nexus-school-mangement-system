...
export function MarkAttendance() {
  ...
  const currentKey = `nexus_attendance_${selectedClass}_${selectedDate}`;
  const [prevKey, setPrevKey] = useState(currentKey);

  const { rosters, saveRoster, saveHistory } = useAttendanceStore();

  const [roster, setRoster] = useState<StudentAttendance[]>(() => {
    return rosters[currentKey] || DEFAULT_ROSTER.map((s) => ({ ...s, status: "Present" }));
  });
  const [isSaved, setIsSaved] = useState(false);

  if (currentKey !== prevKey) {
    setPrevKey(currentKey);
    setRoster(rosters[currentKey] || DEFAULT_ROSTER.map((s) => ({ ...s, status: "Present" })));
    setIsSaved(false);
  }
  ...
  const handleSave = () => {
    const key = `nexus_attendance_${selectedClass}_${selectedDate}`;
