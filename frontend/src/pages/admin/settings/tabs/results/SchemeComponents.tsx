...
import { Plus, AlertCircle, CheckCircle, Edit2 } from "lucide-react";
...
export function SchemeComponents({ schemeId, totalTargetWeight, schemeStatus }: SchemeComponentsProps) {
  ...
  const { loadSchemeComponents, addSchemeComponent, editScheme, editSchemeComponent } = useResultsConfigStore();
  ...
  const startEdit = (comp: AssessmentComponentResponse) => {
  ...
  const handleEditSubmit = async (e: React.FormEvent, componentId: string) => {
    ...
    try {
      const updated = await editSchemeComponent(schemeId, componentId, editForm);
