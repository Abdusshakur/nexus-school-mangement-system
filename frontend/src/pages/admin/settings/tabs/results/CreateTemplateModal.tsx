...
export function CreateTemplateModal({ onClose }: CreateTemplateModalProps) {
  const { createNewSchemeTemplate } = useResultsConfigStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createNewSchemeTemplate({
        name: formData.name,
        total_weight: formData.total_weight,
      });
      toast.success("Global Scheme Template created!");
      onClose();
    } catch (err: any) {
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Define a master template for the whole school
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          ...
          >
            <div className="grid grid-cols-1 gap-6">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Standard 2026 Template"
                />
              </div>
            ...
            <div className="bg-indigo-50 text-indigo-800 p-4 rounded-xl border border-indigo-100 flex gap-3 text-sm">
              ...
              <p>
                After creating this template, you'll be able to define the CA and Exam components, and then apply it to the whole school.
              </p>
            </div>
          </form>
        </div>
