...
import { transferStudent } from "../../../api/students";
...
export function TransferStudentModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Successfully transferred ${studentName}!`);
      onSuccess();
    } catch (err: unknown) {
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Transferring {studentName} to a new class
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 flex flex-col gap-5">
          {!activeContext && (
             <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg flex gap-3 text-amber-800 text-sm">
               ...
               <p>Warning: Cannot fetch active academic session. Transfer may fail.</p>
             </div>
          )}
